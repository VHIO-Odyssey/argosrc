...
#'   sorted by \code{verif_type} and then by \code{verif_fn}, and numbered
#'   sequentially in a \code{verif_num} column. Issue-specific sheets are
#'   written only for rows where \code{n_issues > 0}.
#'
#'   The \code{verif_type} column is derived from \code{verif_fn} using the
#'   following rules, applied in order:
#'   \describe{
#'     \item{\code{"verification_auto"}}{Automatic verifications generated by
#'       [argos_check_verifications()], whose \code{verif_fn} matches the
#'       pattern \code{^verif_<id>_<version>$} (e.g. \code{"verif_1_1"}).}
#'     \item{\code{"completeness_auto"}}{Completeness rows added by
#'       [argos_add_completeness_results()], where \code{verif_fn} equals
#'       \code{"completeness"}.}
#'     \item{\code{"completeness_adhoc"}}{Ad-hoc verifications whose
#'       \code{verif_fn} (i.e. the variable name in the ad-hoc script) starts
#'       with \code{"comp_"}.}
#'     \item{\code{"verification_adhoc"}}{Ad-hoc verifications whose
#'       \code{verif_fn} starts with \code{"verif_"} but does not match the
#'       automatic pattern.}
#'     \item{\code{"undefined_adhoc"}}{Any other ad-hoc verification that does
#'       not match the rules above.}
#'   }
#'
#'   The ordering of \code{verif_type} levels places completeness checks first
#'   (ad-hoc, then auto), followed by verification checks (ad-hoc, then auto),
#'   and finally undefined ad-hoc entries.
#'
  ...
  results_excel <-
    ...
    dplyr::mutate(
      verif_type = dplyr::case_when(
        ...
        .data$verif_fn == "completeness" ~ "completeness_auto",
        stringr::str_detect(
          .data$verif_fn,
          "^comp_"
        ) ~ "completeness_adhoc",
